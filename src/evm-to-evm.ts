...
async function main() {
  ...
  try {
    logger.divider();
    logger.info('Cross-Chain Transfer: Arbitrum -> Avalanche (EVM to EVM)');
    logger.divider();

    // Configuration
    const sourceChain = KanaChainID.Arbitrum;
    const targetChain = KanaChainID.Avalanche;
    ...
    logger.info(`Target Chain: Avalanche (ID: ${targetChain})`);
